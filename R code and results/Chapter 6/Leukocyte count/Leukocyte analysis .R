...
### "birthweight"=df$GEBOORTEGEW_LICHT 
...
### Prepare data + do not penalise covariates
#--------------------------------------------
...
abline(v = -2, lty = 2)
abline(v = -4, lty = 2)
legend("topright", legend = c("PFOS (branched)", "PFHXS (total)", "PFOA (total)", "PFBA",
...
### Density Overlay
#------------------
y_rep <- rbind(mcmc_list$chain1[,279:538],mcmc_list$chain2[,279:538])
...
### Density Overlay
#------------------
y_rep <- cbind(mcmc_list$chain1[,287:546],mcmc_list$chain2[,287:546])
y_rep_long <- data.frame(Value = as.vector(t(y_rep)), Type = "Posterior predictive distribution")
obs_long <- data.frame(Value = obs$Y, Type = "Observed distribution")  # Use log_y
combined_df <- rbind(obs_long, y_rep_long)
ggplot(combined_df, aes(x = Value, color = Type, linetype = Type)) +
...
# Bayesian kernel machine regression gave all exposures PIP of zero, therefore 
# we refit the model without variable selection as an illustration
...
### Plot univariate change fixing all exposures at median values
#---------------------------------------------------------------
pred.resp.univar <- PredictorResponseUnivar(fit = fit_bkmr,y = y, Z = Exp, X = Cov,
                                    center = FALSE, ngrid = 500)


### Plot univariate curve for pfba
#---------------------------------
outpfba <- pred.resp.univar%>%
  filter(variable=="pfba")
outpfba$z <- exp(outpfba$z*0.6274159-1.9412150)
outpfba$est<- outpfba$est-8.174952
p_pfba <- ggplot(outpfba, aes(z, exp(est), ymin = exp(est - 1.96*se), ymax = exp(est + 1.96*se))) + 
  geom_smooth(stat = "identity") + 
  ylab("h(z)")+ylim(0.2,1.5)+xlim(0.01,0.85)+
  theme_minimal()+
  labs(
    x = "PFBA concentrations (µg/L)",
    y = "Multiplicative effect"
  )+
  geom_hline(yintercept = 1, color = "black") +
  geom_vline(xintercept = 0.0950   , linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =0.15 , linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =0.19 , linetype = "dashed", color = "grey40") +
  annotate("text", x =0.0950   , y = 0.5, label = "Q1", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =0.15 , y = 0.5, label = "Q2", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =0.19 , y = 0.5, label = "Q3", vjust = -0.5, hjust = -0.2, size = 4) +
  theme(
    axis.text.y = element_text(size = 12),
    axis.text.x = element_text(size = 12),# Remove x-axis labels
    axis.title.y = element_text(size = 12),    # Move legend to bottom
...
### Plot univariate curve for pfna
#---------------------------------
outpfna <- pred.resp.univar%>%
  filter(variable=="pfna")
outpfna$z <- exp(outpfna$z*0.4389814 -1.3709069)
outpfna$est<- outpfna$est-8.174806
p_pfna <- ggplot(outpfna, aes(z, exp(est), ymin = exp(est - 1.96*se), ymax = exp(est + 1.96*se))) + 
  geom_smooth(stat = "identity") + 
  theme_minimal()+ylim(0.2,1.5)+xlim(0.01,0.89)+
  labs(
    x = "PFNA concentrations (µg/L)",
    y = "Multiplicative effect"
  )+
  geom_hline(yintercept = 1, color = "black") +
  geom_vline(xintercept = 0.1900     , linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =0.2600   , linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =0.3300   , linetype = "dashed", color = "grey40") +
  annotate("text", x =0.1900     , y = 0.5, label = "Q1", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =0.2600   , y = 0.5, label = "Q2", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =0.3300   , y = 0.5, label = "Q3", vjust = -0.5, hjust = -0.2, size = 4) +
  theme(
    axis.text.y = element_text(size = 12),
    axis.text.x = element_text(size = 12),# Remove x-axis labels
    axis.title.y = element_text(size = 12),    # Move legend to bottom
...
### Plot univariate curve for PFOA total 
#---------------------------------------
outlbpfoa  <- pred.resp.univar%>%
  filter(variable=="lbpfoa")
outlbpfoa$z <- exp(outlbpfoa$z*0.4206285+0.1399553 )
outlbpfoa$est<- outlbpfoa$est-8.174346
p_lbpfoa  <- ggplot(outlbpfoa , aes(z, exp(est), ymin = exp(est - 1.96*se), ymax = exp(est + 1.96*se))) + 
  geom_smooth(stat = "identity") + 
  theme_minimal() +ylim(0.2,1.5)+xlim(0.2,6)+
  labs(
    x = "PFOA (total) concentrations (µg/L)",
    y = "Multiplicative effect"
  )+
  geom_hline(yintercept = 1, color = "black") +
  geom_vline(xintercept = 0.92, linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =1.2, linetype = "dashed", color = "grey40") +
  geom_vline(xintercept =1.5, linetype = "dashed", color = "grey40") +
  annotate("text", x =0.92, y = 0.5, label = "Q1", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =1.2, y = 0.5, label = "Q2", vjust = -0.5, hjust = -0.2, size = 4) +
  annotate("text", x =1.5, y = 0.5, label = "Q3", vjust = -0.5, hjust = -0.2, size = 4) +
  theme(
    axis.text.y = element_text(size = 12),
    axis.text.x = element_text(size = 12),# Remove x-axis labels
    axis.title.y = element_text(size = 12),    # Move legend to bottom
...
#--------------------------
combined_plot <- (p_pfba / p_pfna / p_lbpfoa) + 
  plot_annotation(title = "Multiplicative effect on leukocyte count relative to median exposure values")
...
compute_ate <- function(data) {
  ...
  ### Step 4: Predict using full model (with covariates unchanged)
  #---------------------------------------------------------------
  pred_low  <- predict(rf_model, newdata = cf_low)
  pred_high <- predict(rf_model, newdata = cf_high)
  
  
  ### Step 5: Compute average causal effect
  #----------------------------------------
...
compute_ate_lm <- function(data) {
  ...
  ### Step 3: Fit linear model (with interactions) 
  #-----------------------------------------------
  lm_model <- lm(formula(model), data = data)
...
ggplot(df_compare, aes(x = Mean, y = parameter, color = method)) +
  ...
  labs(x = "Average interquartile fold change with 95% confidence or credible interval", y = NULL, 
       color = "Method:", title="Leukocyte count") +
