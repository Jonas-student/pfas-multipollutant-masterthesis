...
# Function to store all the results from WQS regression
# 24/06/2025
################################################################################
...
#---------------------------
run_WQS_bs <- function(obs, bs = 100, signal = "one" , alpha = 0.05, sim_id = 1, psi, q = NULL){
  ...
  coef_WQS <- data.frame(
    ...
    Method = paste0("WQS (",signal, ") regression (rh=1 , bs=", bs, ")"),
    Size = nrow(obs),
