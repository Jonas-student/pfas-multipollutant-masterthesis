...
coefPFASstrong <- subset(coefPFAS, parameter =="pfna"| parameter =="pfda")
...
p_joint_bias <- coefPFASstrong %>%
  ...
  ggplot( aes(x = parameter, y = AbsoluteBias, fill = Method)) +
  ...
  ggtitle("Absolute bias of the individual effect estimate ") +
  ylab("Bias") +
...
p_joint_width <- coefPFASstrong %>%
  ...
  ggplot( aes(x=parameter, y=CI_width, fill=Method)) +
  geom_jitter(aes(color = Method), width = 0.2, alpha = 0.1, show.legend = FALSE) +
  geom_boxplot() +
  scale_fill_manual(values = method_colors) +
  scale_color_manual(values = method_colors) +
  ...
  labs(fill = 'Method:')+
  ggtitle("Width of 95% confidence/credible interval for the individual effect") +
...
name_map <- c("pfda"="PFDA (correlated + effect on outcome)","pfna"="PFNA (correlated + no effect on outcome)",
