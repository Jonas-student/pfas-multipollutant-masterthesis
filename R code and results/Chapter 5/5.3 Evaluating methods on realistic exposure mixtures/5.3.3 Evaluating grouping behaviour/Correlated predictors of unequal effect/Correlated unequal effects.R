...
estimates <-  function(m, seed){
  ...
  sim <- function(n_sim){
    ...
    ### Resample columns that are not of interest (avoiding joint structure)
    #-----------------------------------------------------------------------
    for(m in c(1,2,3,4,7)){
      boot_indices <- sample(1:300, size = n_sim, replace = TRUE)
      simulated_data[,m] <- data_matrix[boot_indices,m]
    }
    ...
    #---------------------------------------------------
    WQS <- 0*X$lbpfos + 0* X$pfba + 1*X$pfda +  
      0*X$pfos + 0*X$lbpfoa + 0*X$pfna + 0*X$lbpfhxs
  ...
  out <- foreach(i=1:m, .combine=rbind, .packages = c("gWQS", "MASS","qgcomp","randomForest","bkmr","BAS","glmnet","nimble","bkmr"))%dopar%{
    ...
    coef_lm <- run_linear_model(obs, alpha = 0.05, sim_id = seed[i], psi = psi)
    
    
    ### Linear model (OLS regression)
    #-------------------------------
    source("~/singleOLS_function_store_results.R")
    coef_singlelm <- run_singlelinear_model(obs, alpha = 0.05, sim_id = seed[i], psi = psi)
    ...
    #--------------------------------
    out <- rbind( coef_bs, coef_bs_abst,coef_rs, coef_WQS_rh,
           coef_WQS_rh_abst, coef_lm, coef_BAS, 
