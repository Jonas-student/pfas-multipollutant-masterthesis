...
final_plot <- annotate_figure(
  final_plot,
  top = text_grob(
    "Simulation results linear additive exposures – Joint effect estimation", 
    face = "bold", size = 16
  )
)
...
coefPFAS <- coefPFAS %>%
  ...
  mutate(
    ...
    parameter = factor(parameter, 
          levels = c("pfba","lbpfoa","lbpfos","lbpfhxs","pfda","pfos","pfna")),
...
coefPFASstrong <- subset(coefPFAS, parameter == "lbpfoa")
...
p_joint_bias <- coefPFASstrong %>%
  ...
  ggplot( aes(x = Method, y = RelativeBias, fill = Method)) +
  geom_jitter(aes(color = Method), width = 0.2, alpha = 0.1, show.legend = FALSE) +
  geom_boxplot() +
  scale_fill_manual(values = method_colors) +
  scale_color_manual(values = method_colors) +
...
p_joint_width <- coefPFASstrong %>%
  ...
  ggplot( aes(x=Method, y=RelativeCIwidth, fill=Method)) +
  geom_jitter(aes(color = Method), width = 0.2, alpha = 0.1, show.legend = FALSE) +
  geom_boxplot() +
  scale_fill_manual(values = method_colors) +
  scale_color_manual(values = method_colors) +
  ...
  guides(fill = guide_legend(nrow = 4)) +  
  labs(fill = 'Method:')+
  ggtitle("Relative width of 95% confidence/credible interval") +
  xlab("") + ylab("Interval width")
...
final_plot <- annotate_figure(
  final_plot,
  top = text_grob(
    "Simulation results for PFOA (total) – Individual linear additive effect estimation (23% of total joint effect)", 
    face = "bold", size = 16
  )
)
...
name_map <- c("pfba" = "PFBA (42%)","lbpfoa" = "PFOA (total) (23%)","lbpfos" = "PFOS (total) (19%)",
              "lbpfhxs" = "PFHXS (total) (9%)", "pfda"="PFDA (5%)","pfos" = "PFOS (2%)","pfna"="PFNA (~0%)")
...
final_plot<-coefPFAS %>%
  ...
  ylab("Posterior inclusion probability (PIP) or bootstrap inlcusion probability (BIP)") +
  labs(fill = 'Method: ')

final_plot <- annotate_figure(
  final_plot,
  top = text_grob(
    "Inclusion probabilities from large to small effects according to joint effect contribution - Linear additve exposures", 
    face = "bold", size = 16
  )
)
