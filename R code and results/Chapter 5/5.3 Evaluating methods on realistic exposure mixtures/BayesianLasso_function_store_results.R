...
# Function to store all the results from Bayesian Lasso
# 18/06/2025
################################################################################
...
#---------------------------
run_BLasso <- function(obs, alpha = 0.05, sim_id = 1, psi){
  
tryCatch({

    ### Define Likelihood & priors
  #-----------------------------
  lassoCode <- nimbleCode({
    ...
    # Vague Gamma prior (shape, rate)
    lambda2 ~ dgamma(0.1, 0.1)  
    
    
    # Local shrinkage parameters and beta priors
    for (j in 1:(p-5)) {
      tau[j] ~ dexp(lambda2 / 2)  # Exponential prior on tau^2
      beta[j] ~ dnorm(0, sd = sigma * sqrt(tau[j]))  # Conditional on sigma
    }
  ...
  #-----------------
  my.inits <-  list(
    list(beta0 = 0, beta = rep(0, p-1), log_sigma = 0, lambda2 = 1, tau = rep(1, p-5)),
    list(beta0 = 0.5, beta = rep(-1, p-1), log_sigma = 0, lambda2 = 1, tau = rep(1, p-5))
  )
  
  
  ### Specify parameters to monitor
  #--------------------------------
  parameters <- c("beta0","beta","sigma","lambda2", "sum_beta","tau")
  ...
  model.sim_lasso <- nimbleMCMC(code = lassoCode,
                                data = my.data,
  ...
  #-------------------------
  coefs <- as.data.frame(round(model.sim_lasso$summary$all.chains,3))
  rownames(coefs) <-c(names(obs)[-1], "Intercept", "lambda2", "sigma","sum_beta",
                      "tau1","tau2","tau3","tau4","tau5","tau6","tau7")
  
  
  ### Joint effect summary
  #-----------------------
  joint_effect <- coefs[15,1]
  empirical_power <- as.numeric(coefs[15, 4] > 0 | coefs[15, 5] < 0)
  CI_coverage <- as.numeric(coefs[15, 4] <= psi[1] & coefs[15, 5] >= psi[1])
  ci_width <- as.numeric(coefs[15, 5] - coefs[15, 4])
  ...
  #----------------------------------
  coef_BayLasso <- data.frame(
    Estimate = estimates,
    Sim = as.factor(sim_id),
    parameter = parameters,
    Method = "Bayesian Lasso regression",
    Size = nrow(obs),
  ...
  return(coef_BayLasso)
  
}, error = function(e) {
  message("Error in run_BLasso (sim_id ", sim_id, "): ", e$message)
  return(NULL)
  })
