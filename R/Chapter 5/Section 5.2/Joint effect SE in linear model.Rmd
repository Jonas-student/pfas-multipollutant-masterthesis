...
# Variance under multicollinearity
# 09/06/2025
################################################################################
...
setwd("~/Projects/SPEC/wp7/T7_3_1/Rcode_master_thesis_Jonas_Meijerink/Joint effect SE/")
```
...
for (n in sample_sizes) {
  for (rho in correlations) {
    ...
    for (sim in 1:n_sim) {
      ...
      one_vec <- rep(1, nrow(vcov_mat))
      var_sum <- as.numeric(t(one_vec) %*% vcov_mat %*% one_vec)
      
      
      if (var_sum < 0) {
        warning("Variance of sum is negative, setting to NA or zero")
        se_sum_beta <- NA
        power_sum <- NA
      } else {
...
## Visualse the variance across the correlation
...
for (n in sample_sizes) {
  for (rho in correlations) {
    ...
    for (sim in 1:n_sim) {
      ...
      df_cont <- as.data.frame(X)
      names(df_cont) <- paste0("X", 1:7)
      df_cont$y <- Y  # Fixed: Use Y instead of y
      model_cont <- lm(y ~ ., data = df_cont)
...
A common issue with mixtures is that they are highly correlated. A one-unit change in one exposure, holding the other exposure constant, may not be well supported by the data and depend heavily on model extrapolation.
...
### Scale effect arrows to one-unit increases
#--------------------------------------------
