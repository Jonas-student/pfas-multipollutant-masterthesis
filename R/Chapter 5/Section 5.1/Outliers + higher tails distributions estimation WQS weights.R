...
#---------------------------
run_WQS_rh_conf <- function(obs, rh = 10, bs = 50, signal = "one" , alpha = 0.05, 
...
### Function to store output
#---------------------------
run_WQS_rh <- function(obs, rh = 50, bs = 10, signal = "one" , alpha = 0.05, sim_id = 1, psi, q = NULL, rho){
...
for (n in sample_sizes) {
  for (rho in correlations) {
    ...
    for (i in 1:n_sim) {
      ...
      ### Use a WQS inspired model to simulate the outcome
      #---------------------------------------------------
      WQS <- 0.186305814*X$lbpfos + 0.051494073*X$pfda + 0.421321419  * X$pfba + 0.020734543        * X$pfos + 0.232626663  * X$lbpfoa + 0.002444214  * X$pfna + 0.085073273  * X$lbpfhxs
...
#--------------------------------------
coefPFAS <- subset(scenarioa, parameter == "lbpfos" | parameter == "pfba" |
                 parameter == "pfda" |parameter == "lbpfhxs" |
                parameter == "pfna" |parameter == "lbpfoa"|parameter == "pfos")
...
### Force the order of Quantile levels
#-------------------------------------
coefPFAS$Quantile <- factor(coefwqs$Quantile, levels = c("Quartile", "Decile", "Continuous"))
coefPFAS$Correlation <- factor(coefwqs$Correlation, levels = c("No correlation", "High correlation"))
...
#-----------------------------
coefPFASoutlier <- subset(resultsoutlier, parameter == "lbpfos" | parameter == "pfba" |
                 parameter == "pfda" |parameter == "lbpfhxs" |
                parameter == "pfna" |parameter == "lbpfoa"|parameter == "pfos")
